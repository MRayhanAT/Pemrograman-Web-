...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['id_mobil']) && !empty($_POST['id_mobil'])) {
        ...
        if ($foto_mobil) {
            ...
            $update_query = "UPDATE mobil SET foto_mobil = ?, nama_mobil = ?, deskripsi = ? WHERE id_mobil = ?";
            $stmt = $conn->prepare($update_query);
            $stmt->bind_param("sssi", $foto_mobil, $nama_mobil, $deskripsi, $id_mobil);
        } else {
            $update_query = "UPDATE mobil SET nama_mobil = ?, deskripsi = ? WHERE id_mobil = ?";
            $stmt = $conn->prepare($update_query);
    ...
    if ($_GET['action'] === 'delete') {
        $id_mobil = $_GET['id'];
        $delete_query = "DELETE FROM mobil WHERE id_mobil = ?";
